...
function toggleRow(key, itemId) {
    expandedRow.value = expandedRow.value === key ? null : key;
    emit("rowClicked", expandedRow.value, itemId);
}
...
<template>
    <Transition>
        ...
        >
            <div v-if="addFilters" class="flex flex-row flex-wrap sm:flex-nowrap justify-start px-4 sm:px-0">
                <div
                    ...
                    <slot name="table">
                        <table class="min-w-full divide-y-2">
                            ...
                            <slot name="body" :show="show">
                                <!-- Main table rows -->
                                <template v-for="(item, index) in resourceData" :key="`table-${name}-row-${index}`">
                                    <tr
                                        class="hover:bg-indigo-100"
                                        :class="typeof props.rowClass === 'function' ? props.rowClass(item, index) : ''"
                                        @click="toggleRow(index, item.id)"
                                    >
                                        <td
                                            v-for="column in queryBuilderProps.columns"
                                            v-show="show(column.key)"
                                            :key="`table-${name}-row-${index}-column-${column.key}`"
                                            class="whitespace-nowrap px-3 py-4 text-sm"
                                            :class="hasExpandedRow && expandedRow === index ? ' font-bold' : ''"
                                        >
                                    ...
                                    <!-- Expanded row -->
                                    <tr
                                        v-if="hasExpandedRow && expandedRow === index"
                                        :key="`table-${name}-expanded-row-${index}`"
                                        class="bg-white"
                                    >
                                        <td :colspan="queryBuilderProps.columns.length" class="px-3 py-4 text-sm">
                                            <div
                                                class="p-4 rounded-xl bg-gray-50 w-full h-full border border-1 border-gray-300">
                                                <slot
                                                    name="expandedRow"
                                                    :item="item"
                                                />
